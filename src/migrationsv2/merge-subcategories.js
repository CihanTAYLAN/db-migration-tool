...
slug'ların '_' karakterinden split edildiğindeki ilk parçadan 2 tane kategori var ise,
bu kategoriler teke düşürülüp içlerindeki ürünler birleştirilir.

## Mantık
1. category_translations tablosundan tüm slug'ları al
2. Her slug'ı '_' ile split et ve ilk parçayı al (prefix)
3. Aynı prefix'e sahip kategorileri grupla
4. Eğer bir prefix altında 2+ kategori varsa:
   - İlk kategoriyi "ana kategori" olarak tut
...
class SubcategoryMerger {
    ...
    async mergeSubcategories() {
        ...
        // Tüm kategorileri ve slug'larını al
        const categoriesQuery = `
            ...
            WHERE ct.slug IS NOT NULL AND ct.slug != ''
            ORDER BY ct.slug
        `;

        const categories = await this.targetDb.query(categoriesQuery);
        logger.info(`Found ${categories.length} categories with slugs`);

        if (categories.length === 0) {
            logger.warning('No categories found with slugs');
            return;
        }

        // Code'ları prefix'e göre grupla (code: george-iii_31 gibi)
        const prefixGroups = new Map();

        for (const category of categories) {
            const prefix = category.code.split('_')[0]; // İlk parçayı al
        ...
        for (const [prefix, categoryGroup] of prefixGroups) {
            if (categoryGroup.length >= 2) {
                ...
                await this.mergeCategoriesForPrefix(prefix, categoryGroup);
                totalMerged += categoryGroup.length - 1; // Ana kategori hariç
            }
        }
    ...
    async mergeCategoriesForPrefix(prefix, categories) {
        try {
            // İlk kategoriyi ana kategori olarak seç (alfabetik olarak)
            ...
            logger.info(`Main category: ${mainCategory.title} (${mainCategory.slug})`);
            ...
            for (const categoryToMerge of categoriesToMerge) {
                ...
                logger.info(`Deleted category: ${categoryToMerge.title} (${categoryToMerge.slug})`);
            }
