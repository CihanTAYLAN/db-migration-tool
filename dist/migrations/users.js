...
async function execute(mysqlClient, postgresClient) {
    try {
        ...
        // Transform uygula
        const transformedData = await exports.usersConfig.transform(data);
        ...
        const values = transformedData.map((row) => `(${columns.map(col => {
            const value = row[col];
